// lib/__tests__/featured.strict.spec.ts
...
describe("getFeaturedTools (STRICT featured)", () => {
  const base = [
    ...
    { slug: "c" }, // not featured at all
    { slug: "d", featured: true }, // legacy top-level
  ...
  it("attaches __videoId only when resolvable", () => {
    ...
    // legacy featured without video ok
  ...
  it("sorts by weight desc, then slug asc", () => {
    const out = getFeaturedTools(base as any, 10);
    const slugs = out.map(x => x.slug);
    // weights: a=5, d=0, e=0 -> a should come first
    expect(slugs[0]).toBe("a");
    // d vs e both weight=0, order by slug asc
